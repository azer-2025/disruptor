...
import com.azer.disruptordemo.disruptor.PictureEditEventProducer;
import com.azer.disruptordemo.domain.User;
import com.azer.disruptordemo.model.PictureEditActionEnum;
import com.azer.disruptordemo.model.PictureEditMessageTypeEnum;
import com.azer.disruptordemo.model.PictureEditRequestMessage;
import com.azer.disruptordemo.model.PictureEditResponseMessage;
...
public class PictureEditHandler extends TextWebSocketHandler {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        log.info("收到用户 {} 的 {} 消息：{}", user.getUserName(), pictureEditMessageTypeEnum, message.getPayload());

        // 处理消息(生产消息到disruptor队列中)
    ...
    public void handleEditActionMessage(PictureEditRequestMessage pictureEditRequestMessage, WebSocketSession session, User user, Long pictureId) throws IOException {
        ...
        if (editingUserId != null && editingUserId.equals(user.getUserId())) {
            log.info("用户 {} 执行 {} 操作", user.getUserName(), actionEnum.getText());
            // 构造响应，发送具体操作的通知
            PictureEditResponseMessage pictureEditResponseMessage = new PictureEditResponseMessage();
